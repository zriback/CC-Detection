...
def main():
    ...
    for run_num, run_transmitter in enumerate(run_choices):
        print(f"Run {run_num + 1}/{total_runs} - {'With' if run_transmitter else 'Without'} transmitter")
        ...
        if run_transmitter:
            transmitter_process = subprocess.Popen(['sudo', './tcp_transmitter_complex.elf'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, preexec_fn=os.setsid)
